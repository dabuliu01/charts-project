...
// 为防止变量名冲突(变量污染) 我们采用立即执行函数策略 (function(){})()
// 注意：多个立即执行函数中间必须加分号隔开
// 监控区域模块制作
(function () {
    // 事件委派 给父元素绑定事件,触发的是子元素
        ...
        series: [
            {
                ...
                // 图表类型
                type: 'pie',
                // 南丁格尔玫瑰图 有两个圈 内圈半径10% 外圈半径70%(百分比必须加引号,像素不加引号不加单位)
                // 饼形图半径,可以是像素,也可以是百分比(基于DOM容器大小)第一项是内半径,第二项是外半径(通过它可以实现饼形图大小)
                radius: ['10%', '70%'],
                // 图表中心位置left50% top50% 距离图表DOM容器
                center: ['50%', '50%'],
                // radius 半径模式,另一种是area面积模式
                roseType: 'radius',
                // 数据集value 数据的值 name 数据的名称
                data: [
                    { value: 20, name: '云南' },
                    { value: 26, name: '北京' },
                    { value: 24, name: '山东' },
